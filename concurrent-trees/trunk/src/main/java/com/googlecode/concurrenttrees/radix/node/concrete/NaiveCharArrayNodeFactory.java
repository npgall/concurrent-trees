...
/**
 * A basic implementation of {@link NodeFactory} which creates {@link NaiveCharArrayNode} objects.
 *
 * @author Niall Gallagher
 */
public class NaiveCharArrayNodeFactory implements NodeFactory {
    ...
    public Node createNode(CharSequence edgeCharacters, Object value, List<Node> childNodes, boolean isRoot) {
        ...
        if (!isRoot && edgeCharacters.length() == 0) {
            throw new IllegalStateException("Invalid edge characters for non-root node: " + CharSequenceUtil.toString(edgeCharacters));
        }
        if (childNodes == null) {
        ...
        return new NaiveCharArrayNode(edgeCharacters, value, childNodes);
    }
